...
Shader "SkyMaster/Water4 SM3.0 Local Lights - Waves WEBGL" {
Properties {
...
CGINCLUDE
	...
	uniform float _ShoreFadeFactor;
	float _TerrainScale;
	...
	half4 frag600( v2f i ) : SV_Target
	{
		...
		half4 edgeBlendFactors = half4(1.0, 0.0, 0.0, 0.0);
			...
			#if defined (SHADER_API_GLES)
				edgeBlendFactors = saturate(-_InvFadeParemeter.x * (depth-i.screenPos.w)); //v3.4 WEBGL FIX 1
				//edgeBlendFactors.y = 1.0-edgeBlendFactors.y; //v3.4 WEBGL FIX 1
			#else
				edgeBlendFactors = saturate(_InvFadeParemeter.x * (depth-i.screenPos.w)); //v3.4 WEBGL FIX 1
				edgeBlendFactors.y = 1.0-edgeBlendFactors.y; //v3.4 WEBGL FIX 1
			#endif
			...
			//edgeBlendFactors = saturate(_InvFadeParemeter.x * (depth-i.screenPos.w));
	...
	v2f vert(appdata_full v)
	{
		...
		float4 tex = tex2Dlod(_ShoreContourTex,float4(ScaledTexPoint,0,0));//v3.4 WEBGL FIX 1
		float diff = 1-(tex.r/2-0.00000001);
		#if defined (SHADER_API_GLES)
			if(_ShoreFadeFactor > 0.000000001){
				v.vertex.xyz += float3(offsets.x, offsets.y *(pow(diff,_ShoreFadeFactor)),offsets.z);//v3.4 WEB GL needs 1-tex.r than tex.r !!!
			}else{
				v.vertex.xyz += float3(offsets.x, offsets.y,offsets.z);//v3.4 WEB GL needs 1-tex.r than tex.r !!!
			}
		#else
			v.vertex.xyz += float3(offsets.x, offsets.y *(pow((tex.r+0.00000001),_ShoreFadeFactor)),offsets.z);//v3.4 WEB GL needs 1-tex.r than tex.r !!!
		#endif
		//v3.4
	//	v.vertex.xyz += float3(offsets.x, offsets.y,offsets.z); //v3.4 WEBGL FIX 2	
