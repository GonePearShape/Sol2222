...
public class VolumeClouds_SM : MonoBehaviour {
	...
	public void convertToShuriken(){
		...
		//GRAB DATA is exist
		ParticleEmitter EmitterLegacy = this.gameObject.GetComponent<ParticleEmitter> ();
		//float passMinSize = 50;
		float passMaxSize = 100;
		float passCount = 400;
		//float passEnergy = 200;
		if(EmitterLegacy != null){
			//passMinSize = EmitterLegacy.minSize;
			passMaxSize = EmitterLegacy.maxSize;
			passCount = EmitterLegacy.maxEmission;
			//passEnergy = EmitterLegacy.maxEnergy;
		}
		ParticleRenderer Rrend = this.gameObject.GetComponent<ParticleRenderer> ();
		Material passMat = null;
		if (Rrend != null) {
			passMat = Rrend.sharedMaterial;
		}

		//Disable Legacy or erase them
		if (EmitterLegacy != null) {
			DestroyImmediate (EmitterLegacy,true);
		}
		if (Rrend != null) {
			DestroyImmediate (Rrend,true);
		}

		//check if Shuriken exists
		ParticleSystem TMPPS = this.gameObject.GetComponent<ParticleSystem> ();		
			if (TMPPS==null){ //this.gameObject.GetComponent<ParticleSystem> () == null) {				
				TMPPS = this.gameObject.AddComponent<ParticleSystem> ();
			} else {//v3.4.5b
				if (EmitterLegacy == null) {
					passMaxSize = TMPPS.main.startSize.constant;
					ParticleSystem.MainModule MainMod = TMPPS.main; //v3.4.9
					passCount = MainMod.maxParticles;//v3.4.9
				}
			}
		if (passMat != null) {
			TMPPS.gameObject.GetComponent<Renderer> ().material = passMat;
			TMPPS.gameObject.GetComponent<ParticleSystemRenderer> ().maxParticleSize = 1;
		}
		ParticleSystem.MainModule MainMod1 = TMPPS.main; //v3.4.9
		MainMod1.maxParticles = (int)passCount;//v3.4.9
		//TMPPS.startSize = passMaxSize;
		ParticleSystem.MinMaxCurve Curve =	MainMod1.startSize;
		Curve.constant = passMaxSize;

		//TMPPS.main.startLifetime = 100000;
		ParticleSystem.MinMaxCurve Curve1 =	MainMod1.startLifetime; //v3.4.9
		Curve1.constant = 100000;

		MainMod1.simulationSpace = ParticleSystemSimulationSpace.World; //v3.4.9
		this.useShuriken = true;

		//v3.4.5b - shaping
		ParticleSystem.ShapeModule Shaper = TMPPS.shape;
		Shaper.shapeType = ParticleSystemShapeType.Box;
		Shaper.scale = new Vector3 (111, 1, 111);
