...
public class SKYMASTER_DemoV34WEBGL : MonoBehaviour {
	...
	void OnGUI() {
			...
			//BOAT SPEED
				...
				//EXTRA CONTROLS
				if(1==1){
					if (GUI.Button(new Rect(10*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+50+50, BOX_WIDTH, 20), "Reset params")){
					...
					if (GUI.Button(new Rect(10*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+50+50+25, BOX_WIDTH, 20), "Water params")){
						if(offsetsON){
							offsetsON = false;
						}else{
							offsetsON = true;
					...
					if(offsetsON){
						...
						WaterHandler.ShoreBlendOffset = GUI.HorizontalSlider(new Rect(9*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+25+50+50+4*50, BOX_WIDTH*3, 20),WaterHandler.ShoreBlendOffset,-1.155f,1.1f);//-0.031f
						...
						//v3.4 - WEBGL
						GUI.TextArea( new Rect(9*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+50+50+6*50, BOX_WIDTH*2, 20),"Shore Waves height control");
						WaterHandler.ShoreWavesFade = GUI.HorizontalSlider(new Rect(9*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+25+50+50+6*50, BOX_WIDTH*3, 20),WaterHandler.ShoreWavesFade,0,5f);
					}
				}
