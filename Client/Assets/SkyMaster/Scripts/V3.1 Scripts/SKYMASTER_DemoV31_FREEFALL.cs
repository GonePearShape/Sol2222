...
public class SKYMASTER_DemoV31_FREEFALL : MonoBehaviour {
	...
	void OnGUI() {
			...
			if (SUNMASTER.Current_Time > 22 | SUNMASTER.Current_Time < 9) {
			
				//SUNMASTER.Current_Time = 9;
				//SUNMASTER.SPEED = 1;
			...
			if(HUD_ON){	
				...
				if (GUI.Button(new Rect(4*(BOX_WIDTH+XOffset), 0, BOX_WIDTH*2, 22), "Toggle Depth cues")){
					if (DepthCue1.activeInHierarchy) {
						DepthCue1.SetActive (false);
						DepthCue2.SetActive (false);
					} else {
						DepthCue1.SetActive (true);
						DepthCue2.SetActive (true);
					}			
				}
			...
			SUNMASTER.SPEED = SPEED;
				...
				SUNMASTER.Max_sun_intensity = GUI.HorizontalSlider(new Rect(2, 1*BOX_HEIGHT+25+50, 100-2, 15),SUNMASTER.Max_sun_intensity,0.5f,3.50f);
			...
			if (windowsON == 0 | windowsON == 3) {
				...
				float min_height = 8800f;
				Camera_up = GUI.HorizontalSlider (new Rect (6 * (BOX_WIDTH + XOffset), 1 * BOX_HEIGHT + 25 + 25, BOX_WIDTH + 315, 30), Camera.main.transform.position.y, min_height, 11850);			
...
//				SUNMASTER.windZone.windMain  = GUI.HorizontalSlider(new Rect(4*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+25, BOX_WIDTH+0, 30),SUNMASTER.windZone.windMain ,0,24);		
			...
			//time of day
				...
				TOD = GUI.HorizontalSlider(new Rect(1*(BOX_WIDTH+XOffset), 1*BOX_HEIGHT+25, BOX_WIDTH, 20),SUNMASTER.Current_Time,0f,23.9f);
